...
本次实验通过 5 个测试用例，从任务完成率、Token 开销、召回精准度、知识容量四个维度量化验证了三层记忆系统的优化效果。TC-02（多轮一致性）需要接入真实 LLM，本报告以定性分析呈现。
...
**定性分析**（需接入真实 LLM 会话，本次未自动化运行）

**实验设计**：预写 20 条记忆，覆盖 4 个知识域（Python 工具链、JS/TS 工具链、部署、数据库），额外插入 4 条**跨域干扰项**（例如"前端项目有 Python 辅助脚本，历史上用过 flake8，现已废弃"）。进行 6 轮交替问答，每轮问题只关联其中一个域。

| 模式 | LLM 可见的记忆条目 | 干扰项是否可见 | 预期 MCS |
|------|-----------------|--------------|---------|
| Mode A | 全部 20 条（含 4 条干扰项） | **是** | ~67–83% |
| Mode B | 每轮 top-5 相关条目 | **否**（相似度 < 0.5，被过滤）| ~100% |

**关键机制**：Mode B 召回块只注入 API 发送的 `user_message`，不修改 `persist_user_message`（[conversation_loop.py:327-329](../agent/conversation_loop.py#L327-L329)），对话历史快照保持纯净，不会被召回块污染。
