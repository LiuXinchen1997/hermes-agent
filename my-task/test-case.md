...
# 每条 ~88 字符，11 条 ≈ 968 字符；Python 工具链，与 QUERY 有关键词重叠（Jaccard 可命中）
ENTRIES = [
...
```
Mode A system_prompt_memory_chars : ~990
Mode B per_turn_recalled_chars    : ~480   (hits=5)
每轮 context 节省                 : ~51%
```
...
# 50 条各不相同的事实（每条 ~91 字符，总计 ~4550 字符，远超 Mode A 上限 2200）
# 格式: "rule_NN: <topic>; this is a hard rule enforced by CI; all teams must align by Q3"
FACTS = [
...
```
Mode A: 成功写入 ~24/50，可检索 ~24 条   ← 超过 2200 字符后全部 reject
Mode B: 成功写入 50/50，T1 热区 ~33 条，T2 冷存 ~17 条，总计 50 条
```
...
| TC-03A Token 开销 | `format_for_system_prompt → None` | 每轮注入字符 | ~990 | ~480（-51%）|
...
| TC-05 容量扩展 | T1→T2 驱逐 + 冷存 | 50 次写入后可检索数 | ~24 条 | **50 条** |
